...
async function getCurrentPosition() {
    return new Promise((resolve, reject) => {
        ...
        const success = (position) => {
            resolve(position);
            document.getElementById('location-status').textContent = 'تم تحديد موقعك بنجاح';
            document.getElementById('location-status').style.color = 'green';
        };

        const error = (err) => {
            console.warn(`خطأ في تحديد الموقع (${err.code}): ${err.message}`);
            document.getElementById('location-status').textContent = 'فشل في تحديد الموقع. يرجى السماح بالوصول إلى الموقع من إعدادات المتصفح';
...
async function requestNotificationPermission() {
    ...
    if (!('serviceWorker' in navigator)) {
        alert('متصفحك لا يدعم التنبيهات المتقدمة');
        return false;
    }
    ...
    try {
        ...
        if (permission === 'granted') {
            document.getElementById('notification-status').textContent = 'تم تفعيل التنبيهات بنجاح';
            document.getElementById('notification-status').style.color = 'green';
            return true;
        } else {
            alert('يجب السماح بالتنبيهات لتلقي تنبيهات الصلاة');
            document.getElementById('notification-status').textContent = 'لم يتم السماح بالتنبيهات';
        ...
        console.error('خطأ في طلب إذن التنبيهات:', err);
        document.getElementById('notification-status').textContent = 'حدث خطأ في تفعيل التنبيهات';
        document.getElementById('notification-status').style.color = 'red';
        return false;
    }
}
